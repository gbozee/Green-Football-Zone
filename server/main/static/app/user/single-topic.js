(function () {
    ...
    angular.module('app').controller(controllerId, ['$filter','$routeParams','common','config',
        'ReplyService','userInfo','Conf','Pusher','ngProgress', topic]);
    angular.module('app').service('ReplyService', ['gfService','$angularCacheFactory','$rootScope', ReplyService]);
    ...
    function ReplyService(gfService,$angularCacheFactory,$rootScope){
        ...
        service.getTopicId = function(){return topicId;};
        ...
        service.getRepliesForTopic = function (team){
           return topicService.one(team).all('replies').getList()
               .then(function(data){
                   replies = data;
                   return replies;
               });
    ...
    function topic($filter,$routeParams,common,config, ReplyService,userInfo,Conf,Pusher,ngProgress) {
        ...
        function activate() {
            common.activateController([getTopic(),getReplies()], controllerId)
                .then(function () {
        ...
        function getTopic() {
            return ReplyService.getTopic($routeParams.topic_id)
                .then(function (data) {
                    console.log(data);
                    vm.topic = data;
        ...
        function addReply($event) {
            ...
            if ($event.type === 'click' || $event.keyCode === keyCodes.enter) {
                ...
                ngProgress.complete();
                ngProgress.start();
                return userInfo.addReply(vm.topic,vm.newReply)
                    .then(function (reply) {
