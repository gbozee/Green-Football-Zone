(function () {
    ...
    angular.module('app').controller(controllerId, ['$filter','$routeParams','common', 'config','userInfo', user_replies]);
    angular.module('app').factory('userInfo', ['$rootScope','common','$filter','Restangular','$q','ngProgress',userInfo]);
...
//             $http.post('/api/v2.0/connect',authResult).then(function(data){
//                 console.log(data);
//             },function(error){
//                 console.log(error);
//             });
...
//
//        myResource.customGETLIST('comments').then(function(data){
//            setReplies(data);
//        });
...
//                user.followed_topics[index] = topic;
            }
        };
        ...
        userService.followTeam = function(team){
            ...
            teamResource.one(team.name).customGET('follow')
                .then(function(data){
                    console.log(data);
                    teams.push(team);
//                    user.teams.push(team);
                ...
                .then(function(){
                    myResource.customGETLIST('topics')
                        .then(function(data){
//                            user.followed_topics = data;
                            followed_topics = data;
...
//            datacontext.unfollowTeam(team.name).then(function(data){
...
//                user.teams.splice(index,1);
        ...
        userService.addReply = function(topic,reply) {
            ...
            topicResource.one(topic.id).post('replies',reply)
//            datacontext.createReply(topic, reply)
                .then(function (reply) {
                    replies.push(reply);
...
//            datacontext.deleteReply(reply).then(function(){
...
//            datacontext.editReply(reply).then(function(){
                var index = _.findIndex(replies,{id:reply.id})
                replies[index] = reply;
                log('Reply Updated');
                deferred.resolve(reply);
...
//            datacontext.updateAuthoredTopics(topic).then(function(data){
                var index = _.findIndex(authored_topics,{id:topic.id})
    ...
    function user_replies($filter,$routeParams,common, config,userInfo) {
        ...
        function activate() {
            common.activateController([getAllReplies(true)], controllerId)
                .then(function () {
        ...
        function replyFilter(reply) {
            ...
            var isMatch = searchText ?
                textContains(reply.body, searchText)
                    || textContains(reply.topic, searchText)
                    || textContains(reply.body, searchText)
                    || textContains(reply.created, searchText)
                    || textContains(reply.votes, searchText)
                : true;
            return isMatch;
...
//            return datacontext.deleteReply(reply).then(function(){
//                var index = _.findIndex(vm.replies,{id:reply.id})
