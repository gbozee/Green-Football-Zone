...
//    angular.module('app').controller(controllerId, ['common', 'leagueResource','teamResource','userResource', admin]);
    angular.module('app').controller(controllerId, ['common', 'gfUtils', 'adminService','Pusher', admin]);
//    angular.module('app').controller('parginationCtrl', ['vm','$filter', paginationCtrl]);
    ...
    angular.module('app').service('adminService', ['Restangular','$angularCacheFactory', adminService]);
...
//            return leagueResource.one(league.name).post(team).then(function (data) {
//                teams.push(data);
//                return data;
...
//                vm.pagedTeamItems = data;
    ...
    function gfTeamListDirective() {
        return {
            restrict: 'A',
            template: '<tr data-ng-repeat="p in list">' +
                '<td><a href="#/team_update/{{ p.name }}">{{ p.name }}</a></td>' +
                '<td>{{ p.logo }}</td>' +
                '<td>{{ p.admin }}</td>' +
                '</tr>',
            scope: {
    ...
    function gfLeagueListDirective() {
        return {
            restrict: 'A',
            template: '<tr data-ng-repeat="p in list">' +
                '<td><a href="#/league_update/{{ $index }}">{{ p.name }}</a></td>' +
                '<td>{{ p.logo }}</td>' +
                '</tr>',
            scope: {
                list: '='
