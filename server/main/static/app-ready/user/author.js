(function () {
  ...
  angular.module('app').controller('userDetails', [
    'common',
    'gfService',
    'userInfo',
    userDetails
  ...
  function userDetails(common, gfService) {
    ...
    activate();
    function activate() {
      common.activateController([getUserDetails()], 'userDetails').then(function () {
  ...
  function author(bsDialog, config, $filter, $routeParams, $location, userInfo, common, inhouse, ngProgress) {
    ...
    activate();
    function activate() {
      common.activateController([getAuthoredTopics(true)], controllerId).then(function () {
    ...
    function addTopic($event) {
      ...
      if ($event.type === 'click' || $event.keyCode === keyCodes.enter) {
        ngProgress.complete();
        ngProgress.start();
        return userInfo.addTopic(vm.topic).then(function (topic) {
          log('Topic Added');
          getAuthoredTopics();
          vm.topic = {};
          doneCreating();
    ...
    function deleteTopic(topic) {
      ngProgress.start();
      return userInfo.deleteTopic(topic).then(function () {
        ngProgress.complete();
        log('Topic Deleted');
        getAuthoredTopics();
      });
    ...
    function topicsFilter(topic) {
      ...
      var isMatch = searchText ? textContains(topic.title, searchText) || textContains(topic.body, searchText) || textContains(topic.team, searchText) || textContains(topic.second_team, searchText) : true;
      return isMatch;
    }
  }
}());