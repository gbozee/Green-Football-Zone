(function () {
  ...
  function topics(common, config, $filter, inhouse, userInfo, Pusher, ngProgress) {
    ...
    activate();
    function activate() {
      common.activateController([getUserTopics(true)], controllerId).then(function () {
    ...
    function topicsFilter(topic) {
      ...
      var isMatch = searchText ? textContains(topic.body, searchText) || textContains(topic.author, searchText) || textContains(topic.team, searchText) || textContains(topic.title, searchText) : true;
      return isMatch;
    }
