(function () {
  ...
  angular.module('app').service('TeamService', [
    'gfService',
    '$q',
    '$angularCacheFactory',
    '$rootScope',
  ...
  function TeamService(gfService, $q, $angularCacheFactory, $rootScope) {
    ...
    service.getTeamsFromLeague = function (league) {
      return leagueService.one(league).customGETLIST('teams').then(function (data) {
        teams = data;
        return teams;
      });
    };
  ...
  function teams($routeParams, common, config, userInfo, TeamService, Pusher, $rootScope, ngProgress) {
    ...
    activate();
    function activate() {
      common.activateController([getTeams(false)], controllerId).then(function () {
    ...
    function teamFilter(team) {
      ...
      var isMatch = searchText ? textContains(team.name, searchText) || textContains(team.type, searchText) || textContains(team.league, searchText) || textContains(team.c_league, searchText) || textContains(team.coach, searchText) : true;
      return isMatch;
    }
  }
}());