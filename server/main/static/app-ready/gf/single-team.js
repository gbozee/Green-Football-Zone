(function () {
  ...
  angular.module('app').controller('gfFollowController', [
    'common',
    '$http',
    '$scope',
    'userInfo',
  ...
  function TopicService(gfService, $angularCacheFactory, $rootScope) {
    ...
    service.getTopicsForTeam = function (team) {
      return teamService.one(team).customGETLIST('topics').then(function (data) {
        topics = data;
        return topics;
      });
    };
  ...
  function team($filter, $routeParams, common, TopicService, Conf, userInfo, Pusher, $rootScope, ngProgress) {
    ...
    function followButton() {
      ...
      if (button.hasClass('following')) {
        return teamResource.unfollowTeam(vm.team).then(function (result) {
          vm.button = result;
        ...
        return teamResource.followTeam(vm.team).then(function (result) {
          vm.button = result;
          vm.button.button_class = '';
