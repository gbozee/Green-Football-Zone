(function () {
  ...
  function update_league($routeParams, $location, common, Restangular, gfTypeSupport) {
    ...
    function getLeague() {
      leagueResource.one($routeParams.name).get().then(function (data) {
        console.log(data);
        vm.league = data;
      });
    }
    function updateLeague() {
      leagueResource.one($routeParams.name).customPUT(vm.league).then(function (data) {
        log('League Updated');
        $location.url('/admin');
      });
    }
