(function () {
  ...
  function datacontext($http, common, Conf, $angularCacheFactory, userInfo, LeagueService, TeamService) {
    ...
    function getLeagues(refresh) {
      var deferred = $q.defer(), leagueCache = $angularCacheFactory.get(gfCache);
      if (refresh) {
    ...
    function prime() {
      ...
      primePromise = $q.all([
        ...
        userInfo.getUserTeams(),
        userInfo.fetchUserReplies(),
        userInfo.refreshAuthoredTopics(),
        userInfo.refreshFollowedTopics(true),
        TeamService.getTeams(),
        LeagueService.getLeagues()
      ]).then(extendMetadata()).then(success);
      return primePromise;
      function success() {
        log('Primed the data');
    ...
    function getFeeds(refresh) {
      ...
      var requestUrl = 'http://ajax.googleapis.com/ajax/services/feed/load?v=1.0&callback=JSON_CALLBACK&num=50&q=' + encodeURIComponent('http://greenfootball.org/?feed=rss2');
      var deferred = $q.defer();
    ...
    function getGooglePlusFriends(forceRefresh) {
      ...
      var deferred = $q.defer();
      $http.get(Conf.apiBase + 'friends').then(function (result) {
